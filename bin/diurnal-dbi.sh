APP_NAME="diurnal-dbi-service"
APP_VERSION="0.0.4-SNAPSHOT"
JAVA_PARAM="-Xmx351m"
APP_PARAMS="-DSPRING_SECURITY_USERNAME=$PROM_SPRING_USER -DSPRING_SECURITY_CRED=$PROM_SPRING_CRED -DSPRING_PROFILE=local -DDBI_INACTIVE_TIMEOUT_SECONDS=300 -DCRDB_PORT=$CRDB_PORT -DCRDB_DB=$CRDB_DB"

BIN_PATH=$PROM_HOME_PARENT/Diurnal/$APP_NAME/bin     #PROM-HOME-PARENT :: exported in .bashrc
JAR_PATH=$BIN_PATH/../target/$APP_NAME-$APP_VERSION.jar

echo "Starting '$APP_NAME' with java param: '$JAVA_PARAM', app params: '$APP_PARAMS' at '$JAR_PATH'"
java $JAVA_PARAM $APP_PARAMS -jar $JAR_PATH
